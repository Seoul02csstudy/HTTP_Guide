...
## 3.3 메서드
- HTTP는 안전한 메서드라 불리는 메서드의 집합
- 서버 제작 시 서버에 어떤 영향을 줄 수 있는 안전하지 않은 메서드가 사용될 때, 사용자들에게 그 사실을 알려주는 HTTP 애플리케이션을 제작해야 함 -> **안전한 메서드** 
- 메서드의 종류
  - GET
    - 서버에게 리소스를 달라고 요청하기 위해 쓰임
  - HEAD 
    - 서버에게 GET과 같이 리소스를 요청하지만, 서버는 응답으로 헤더만 제공
    - **장점**
      - 해당 타입을 알아낼 수 있음
      - 개체 존재 여부 확인 가능
      - 리소스 변경 확인 가능
  ...
  - POST
    - 서버에 입력 데이터를 전송
    - HTML 폼을 지원하기 위해 흔히 사용(게시물 작성)
  - TRACE
    - 클라이언트에게 자신의 요청이 서버에 도달했을 때, 어떻게 보이는지 알려줌
    - 클라이언트가 한 요청이 의도된 요청이나 응답 연쇄를 거쳐가는지 검사 가능
    - 단, 중간 어플리케이션이 여러가지 다른 종류의 요청들을 일관되게 다뤄야 한다고 가정해야 하는 문제가 있음
  - OPTIONS
    - 웹 서버에게 여러가지 종류의 지원범위를 물어봄
    - 서버는 자신의 리소스에 대해 지원하는 메서드 목록 반환 ex) GET,POST,PUT 제공..
  - DELETE
    - 요청 URL로 지정한 리소스 삭제
  - 확장 메서드
    - HTTP는 필요에 따라 확장해도 문제가 없음
    - 단, 모든 확장 메서드가 형식을 갖춘 명세로 정의된 것은 아님


      | method  | 설명                                                   |
      | ------- | ------------------------------------------------------ |
      | LOCK     | 사용자가 리소스 잠금 -> 동시편집 막기                         |
      | MKCOL   | 사용자가 문서 생성하도록 해줌 |
      | COPY   | 서버에 있는 리소스 복사              | 
      | MOVE   | 서버에 있는 리소스 이동                 |        
...
## 3.5 헤더
  - 클라이언트와 서버가 무엇을 하는지 결정하기 위해 메서드와 함께 사용됨

  | 종류  | 설명                                                   |
  | ------- | ------------------------------------------------------ |
  | 일반헤더     | 클라이언트와 서버 양쪽 모두 사용                          |
  | 요청헤더   | 요청메시지를 위한 헤더 - ex) 데이터타입 |
  | 응답헤더   | 응답메시지가 클라이언트에게 정보 제공을 위해 사용하는 헤더              | 
  | 엔터티헤더    | 본문에 대한 데이터 타입 등 확인해줌                    |
  | 확장헤더     | 명세에 추가되지 않은 비표준 헤더                |
